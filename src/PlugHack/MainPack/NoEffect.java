        ...
        if(command.getLabel().equalsIgnoreCase("noeffect")) {
            if(commandSender instanceof Player) {
                if(commandSender.hasPermission("PlugHack.NoEffect")) {
                    ...
                    if(targetPlayer.getPersistentDataContainer().getOrDefault(new NamespacedKey(plugin,"noeffect"), PersistentDataType.STRING, "false").equals("false")) {
                        commandSender.sendMessage(ChatColor.GREEN + "Enabled NoEffect");
                        targetPlayer.getPersistentDataContainer().set(new NamespacedKey(plugin, "noeffect"), PersistentDataType.STRING, "true");
                    }else {
