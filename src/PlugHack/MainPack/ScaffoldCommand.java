        ...
        if(command.getLabel().equalsIgnoreCase("scaffold")) {
           if(sender instanceof Player) {
             if(sender.hasPermission("PlugHack.Scaffold")) {
                 ...
                 if(Objects.requireNonNull(targetPlayer).getPersistentDataContainer().getOrDefault(new NamespacedKey(plugin,"scaffoldtoggle"), PersistentDataType.STRING, "false").equals("false")) {
                     targetPlayer.getPersistentDataContainer().set(new NamespacedKey(plugin, "scaffoldtoggle"), PersistentDataType.STRING, "true");
