       ...
       if(command.getLabel().equalsIgnoreCase("velocity")) {
           if(commandSender instanceof Player) {
               if(commandSender.hasPermission("PlugHack.Velocity")) {
                   ...
                   if(targetPlayer.getPersistentDataContainer().getOrDefault(new NamespacedKey(plugin,"velocity"), PersistentDataType.STRING, "false").equals("false")) {
                       targetPlayer.getPersistentDataContainer().set(new NamespacedKey(plugin, "velocity"), PersistentDataType.STRING, "true");
